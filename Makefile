# Makefile for the netmon project.
...
CPFLAGS := -p
# GOFLAGS := -ldflags "-X 'github.com/duluk/netmon/pkg/config.commit=$(shell git rev-parse --short HEAD)' -X 'github.com/duluk/ask-ai/pkg/config.date=$(shell date -u '+%Y-%m-%d %H:%M:%S')'"
